...
-- Phase:
data BuildPhase = Init | Core | Optional | Finalized -- explain why in Basic using 5 phases, here only 4

-- ShipState: follow the phases, track reactors and core systems, track used slots
...
-- SINGLETONS: bridge between type-level and value-level
...
-- start_blueprint
start_blueprint :: P.String -> ShipBuilder EmptyState EmptyState ()
start_blueprint name = ShipBuilder $ \_ -> ((), ShipData name [] 0 0 0 0)

-- set_frame: Mass 1000, Slots 10
set_frame :: ShipBuilder EmptyState 
                        (ShipState 'Core '( 'False, 'False) '( 'False, 'False, 'False, 'False) 0) ()
set_frame = ShipBuilder $ \d ->
    ((), d { sMass = 1000, sComps = [CFrame] })

-- add_reactor: Fusion (300m, 1000p) | Antimatter (450m, 1000p) - Cost 3
add_reactor :: (CheckSlots s 3)
...
add_life_support :: (CheckSlots s 2)
               => SLifeSupport lt
               -> ShipBuilder (ShipState 'Core rMap '(hasR, hasE, hasL, hasB) s)
                              (ShipState 'Core rMap '(hasR, hasE, 'True, hasB) (s + 2)) ()
add_life_support lt = ShipBuilder $ \d ->
...
add_bridge :: (CheckSlots s 1)
          => SBridge bt
          -> ShipBuilder (ShipState 'Core rMap '(hasR, hasE, hasL, hasB) s)
                         (ShipState 'Core rMap '(hasR, hasE, hasL, 'True) (s + 1)) ()
add_bridge bt = ShipBuilder $ \d ->
...
lock_core_systems = ShipBuilder $ \d -> ((), d)

-- add_shield: Mass 40 - Draw 100 - Cost 1
add_shield :: (CheckSlots s 1, CheckCompatibility rMap st) -- CheckCompatibility for [B-440]
          => SShield st
          -> ShipBuilder (ShipState 'Optional rMap cores s)
                         (ShipState 'Optional rMap cores (s + 1)) ()
add_shield st = ShipBuilder $ \d ->
...
add_sensors :: (CheckSlots s 1)
           => SSensor st
           -> ShipBuilder (ShipState 'Optional rMap cores s)
                          (ShipState 'Optional rMap cores (s + 1)) ()
add_sensors st = ShipBuilder $ \d ->
...
-- Finalize
finalize_blueprint :: ShipBuilder (ShipState 'Optional rMap cores s)
                        (ShipState 'Finalized rMap cores s) ShipData
finalize_blueprint = ShipBuilder $ \d -> (d, d)
...
-- calculate slots for each component (Runtime)
...
combatShip :: ShipData
combatShip = build $ do
    start_blueprint "Battle Cruiser CBC"
    set_frame
    
    -- Core Modules
    add_reactor SFusion      -- 300 mass, 1000 pwr
    add_engine SPlasma       -- 120 mass, 750 thrust
    add_life_support SStandard
    add_bridge SCommand      -- 60 mass
    lock_core_systems
    
    -- Optional Modules
    add_shield SMagnetic     -- Fusion + Magnetic = OK
    add_sensors SEnhanced  
...
test_A103_Success = build $ do
    start_blueprint "A-103 OK"
    set_frame 
    add_reactor SFusion
    add_engine SIon
    add_life_support SStandard
    add_bridge SExplorer
    lock_core_systems
    finalize_blueprint

-- test_A103_Fail :: ShipData
-- test_A103_Fail = build $ do
--     start_blueprint "A-103 Fail"
--     add_reactor SFusion -- LỖI: Cố lắp Reactor ngay khi chưa set_frame
--     finalize_blueprint
