...
add_reactor r = do
    ...
    unless (currentPhase s == CorePhase) $
        throwError "[A-305] ORDERING ERROR: Core Modules (Reactor) can only be added BEFORE locking core systems."
...
add_engine e = do
    s <- get
    unless (currentPhase s == CorePhase) $
        throwError "[A-305] ORDERING ERROR:Core Modules (Engine) can only be added BEFORE locking core systems."
...
add_life_support l = do
    s <- get
    unless (currentPhase s == CorePhase) $
        throwError "[A-305] ORDERING ERROR:Core Modules (Life Support) can only be added BEFORE locking core systems."
...
add_bridge b = do
    s <- get
    unless (currentPhase s == CorePhase) $
        throwError "[A-305] ORDERING ERROR:Core Modules (Bridge) can only be added BEFORE locking core systems."
...
add_sensor se = do
    ...
    unless (currentPhase s == OptionalPhase) $
        throwError "[A-305] ORDERING ERROR: Optional Modules (Sensor) can only be added AFTER locking core systems."
...
add_shield sh = do
    s <- get
    unless (currentPhase s == OptionalPhase) $
        throwError "[A-305] ORDERING ERROR:Optional Modules (Shield) can only be added AFTER locking core systems."

    let rTypes = [r | Reactor r <- components s] 
    when (sh == Phase && Fusion `elem` rTypes) $
        throwError "[B-440] DEPENDENCY ERROR: Phase Shields are incompatible with Fusion Reactors."
