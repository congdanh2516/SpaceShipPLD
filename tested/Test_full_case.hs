...
design5 :: ShipBuilder Spaceship
design5 = do
    start_blueprint "USS Test5"
    set_frame
    add_reactor Fusion
    add_engine Ion
    add_life_support Standard
    add_bridge Explorer
    lock_core_systems
    add_shield Phase -- illegal with Fusion
    finalize_blueprint

-- Scenario 6: Slot overflow (should fail [B-307])
design6 :: ShipBuilder Spaceship
design6 = do
    start_blueprint "USS Test6"
    set_frame
    add_reactor Fusion        -- 3 slots
    add_engine Plasma         -- 2 slots
    add_life_support Standard -- 2 slots
    add_bridge Explorer       -- 1 slot, total 8
    lock_core_systems
    add_shield Magnetic       -- 1 slot, total 9
    add_sensor Basic          -- 1 slot, total 10
    add_sensor Basic          -- illegal, exceeds 10 slots
    finalize_blueprint

-- Scenario 7: Lock core systems missing one core (should fail [B-209])
design7 :: ShipBuilder Spaceship
design7 = do
    start_blueprint "USS Test7"
    set_frame
    add_reactor Fusion
    add_engine Ion
    add_life_support Standard
...
-- Scenario 8: Multiple Reactors + Shield dependency (should fail [B-440])
design8 :: ShipBuilder Spaceship
design8 = do
    start_blueprint "USS Test8"
    set_frame
    add_reactor Fusion
    add_reactor Antimatter
    add_engine Ion
    ...
    add_bridge Explorer
    lock_core_systems
    add_shield Magnetic -- illegal with Antimatter
    finalize_blueprint

-- Scenario 9: Valid multiple Reactors and optional modules (success)
design9 :: ShipBuilder Spaceship
design9 = do
    start_blueprint "USS Test9"
    set_frame
    add_reactor Fusion
    add_reactor Antimatter
    add_engine Ion
    ...
    add_sensor Basic
    finalize_blueprint
-- Scenario 10: Full valid spaceship (success, prints ASCII)
design10 :: ShipBuilder Spaceship
design10 = do
    start_blueprint "USS Titan"
    set_frame

    -- Core Phase
    add_reactor Fusion        -- 3 slots
    add_engine Ion            -- 2 slots
    add_life_support Advanced -- 2 slots
    add_bridge Command        -- 1 slot  --> Total 8/10
    lock_core_systems         -- move to OptionalPhase

    -- Optional Phase
    add_shield Magnetic       -- 1 slot (Fusion + Magnetic = OK)
    add_sensor Enhanced       -- 1 slot  --> Total 10/10
...
testAll :: IO ()
testAll = do
    let scenarios = [("Scenario 1: Valid Spaceship", designAlpha),
                 ("Scenario 2: Finalize without core", designBeta),
                 ("Scenario 3: Add core before set_frame", design3),
                 ("Scenario 4: Add optional module before lock_core_systems", design4),
                 ("Scenario 5: Shield + Reactor dependency", design5),
                 ("Scenario 6: Slot overflow", design6),
                 ("Scenario 7: Lock core missing one core", design7),
                 ("Scenario 8: Multiple Reactors + Shield dependency", design8),
                 ("Scenario 9: Valid multiple Reactors and optional modules", design9),
                 ("Scenario 10: Successfully construct", design10)]

    forM_ scenarios $ \(desc, design) -> do
        putStrLn $ "\n=== " ++ desc ++ " ==="
        let result = evalState (runExceptT design) initialSpaceship
        print_spec result
